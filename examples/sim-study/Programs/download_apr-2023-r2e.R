...
file.copy('c:/projects/bcf-sim-infra/data/sim_study_config_medicare_and_edu.xlsx',
          'data/apr-2023-r2e/sim_study_config_medicare_and_edu.xlsx',
          overwrite=TRUE)

file.copy('c:/projects/bcf-sim-infra/scripts/apr-2023-r2e/test_list.RDS',
          'data/apr-2023-r2e/test_list.RDS',
          overwrite=TRUE)

scenarios <- readRDS('data/apr-2023-r2e/test_list.RDS') %>%
  ...
  distinct %T>%
  saveRDS('data/apr-2023-r2e/_scenarios.RDS') %>%
  select(scenario, world, set)

from <- 's3://bcf-sim-study/apr-2023-r2e/1kburn-1ksim'
to <- 'C:/Projects/bcf-sim-study/Data/apr-2023-r2e/1kburn-1ksim'
...
files <- tibble(file = dir(to, pattern = '*.RDS', recursive = FALSE, full.names = TRUE)) %>%
  mutate(name = file %>% str_remove('C:/Projects/bcf-sim-study/Data/apr-2023-r2e/1kburn-1ksim/') %>% str_remove('\\.RDS'),
...
combine_output_chunks(smalls, dir='C:/Projects/bcf-sim-study/Data/apr-2023-r2e', save_big=FALSE)
#combine_output_chunks(bigs, dir='C:/Projects/bcf-sim-study/Data/apr-2023-r2e', save_big=TRUE)
