...
file.copy('c:/projects/bcf-sim-infra/data/sim_study_config_medicare_and_edu.xlsx',
          'data/jan-2023-runs/sim_study_config_medicare_and_edu.xlsx',
          overwrite=TRUE)

file.copy('c:/projects/bcf-sim-infra/scripts/jan-2023-runs/test_list.RDS',
          'data/jan-2023-runs/test_list.RDS',
          overwrite=TRUE)

scenarios <- readRDS('data/jan-2023-runs/test_list.RDS') %>%
  ...
  select(-seed, -ibcf, -fname) %>%
  distinct %T>%
  saveRDS('data/jan-2023-runs/_scenarios.RDS') %>%
  select(scenario, world, set)

from <- 's3://bcf-sim-study/jan-2023-runs/1kburn-1ksim'
to <- 'C:/Projects/bcf-sim-study/Data/jan-2023-runs/1kburn-1ksim'
...
files <- tibble(file = dir(to, pattern = '*.RDS', recursive = FALSE, full.names = TRUE)) %>%
  mutate(name = file %>% str_remove('C:/Projects/bcf-sim-study/Data/jan-2023-runs/1kburn-1ksim/') %>% str_remove('\\.RDS'),
         world = str_extract(file,'medicare|edu'),
         scenario = str_match(file,'((medicare|edu)_(.+))_(ibcf|wbcf)')[,2],
...
#combine_output_chunks(bigs, dir='C:/Projects/bcf-sim-study/Data/jan-2022-runs', save_big=TRUE)
combine_output_chunks(smalls, dir='C:/Projects/bcf-sim-study/Data/jan-2023-runs', save_big=FALSE)
