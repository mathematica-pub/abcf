...
file.copy('c:/projects/bcf-sim-infra/data/sim_study_config_medicare_and_edu.xlsx',
          'data/jan-2023-runs/rhohard0/sim_study_config_medicare_and_edu.xlsx',
          overwrite=TRUE)

file.copy('c:/projects/bcf-sim-infra/scripts/feb-2023-rhohard0/test_list.RDS',
          'data/jan-2023-runs/rhohard0/test_list.RDS',
          overwrite=TRUE)

scenarios <- readRDS('data/jan-2023-runs/rhohard0/test_list.RDS') %>%
  ...
  distinct %T>%
  saveRDS('data/jan-2023-runs/rhohard0/_scenarios.RDS') %>%
  select(scenario, world, set)

from <- 's3://bcf-sim-study/jan-2023-runs/1kburn-1ksim-rhohard0'
to <- 'C:/Projects/bcf-sim-study/Data/jan-2023-runs/1kburn-1ksim-rhohard0'
...
files <- tibble(file = dir(to, pattern = '*.RDS', recursive = FALSE, full.names = TRUE)) %>%
  mutate(name = file %>% str_remove('C:/Projects/bcf-sim-study/Data/jan-2023-runs/1kburn-1ksim-rhohard0/') %>% str_remove('\\.RDS'),
...
combine_output_chunks(smalls, dir='C:/Projects/bcf-sim-study/Data/jan-2023-runs/rhohard0', save_big=FALSE)
