...
file.copy('c:/projects/bcf-sim-infra/data/sim_study_config_medicare_and_edu.xlsx',
          'data/mar-2023-wcorr2/sim_study_config_medicare_and_edu.xlsx',
          overwrite=TRUE)

file.copy('c:/projects/bcf-sim-infra/scripts/mar-2023-wcorr2/test_list.RDS',
          'data/mar-2023-wcorr2/test_list.RDS',
          overwrite=TRUE)

scenarios <- readRDS('data/mar-2023-wcorr2/test_list.RDS') %>%
  ...
  distinct %T>%
  saveRDS('data/mar-2023-wcorr2/_scenarios.RDS') %>%
...
files <- tibble(file = dir(to, pattern = '*.RDS', recursive = FALSE, full.names = TRUE)) %>%
  ...
  filter(str_detect(name,'wcovar')) %>%
  left_join(scenarios, by=c('world','scenario'))

smalls <- filter(files, str_detect(file,'small'))
bigs <- filter(files, !str_detect(file,'small'))

#Fuuuuck I forgot to save w_as_covar in inputs
if (FALSE) {
  for (f in files$file) {
    w <- as.logical(str_match(f,'wcovar(TRUE|FALSE)')[,2])
    x <- readRDS(f)
    x$inputs$w_as_covar <- w
    saveRDS(x,f)
  }
}

combine_output_chunks(smalls, dir='C:/Projects/bcf-sim-study/Data/mar-2023-wcorr2', save_big=FALSE)
