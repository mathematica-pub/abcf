...
file.copy('c:/projects/bcf-sim-infra/data/sim_study_config_medicare_and_edu.xlsx',
          'data/feb-2023-nonlin/sim_study_config_medicare_and_edu.xlsx',
          overwrite=TRUE)

file.copy('c:/projects/bcf-sim-infra/scripts/feb-2023-nonlin/test_list.RDS',
          'data/feb-2023-nonlin/test_list.RDS',
          overwrite=TRUE)

scenarios <- readRDS('data/feb-2023-nonlin/test_list.RDS') %>%
  ...
  select(-seed, -ibcf, -ubcf, -obcf, -fname, -isstan, -ate_prior_sd) %>%
  distinct %T>%
  saveRDS('data/feb-2023-nonlin/_scenarios.RDS') %>%
  select(scenario, world, set)

from <- 's3://bcf-sim-study/feb-2023-nonlin/1kburn-1ksim'
to <- 'C:/Projects/bcf-sim-study/Data/feb-2023-nonlin/1kburn-1ksim'
...
files <- tibble(file = dir(to, pattern = '*.RDS', recursive = FALSE, full.names = TRUE)) %>%
  mutate(name = file %>% str_remove('C:/Projects/bcf-sim-study/Data/feb-2023-nonlin/1kburn-1ksim/') %>% str_remove('\\.RDS'),
...
#combine_output_chunks(smalls, dir='C:/Projects/bcf-sim-study/Data/feb-2023-nonlin', save_big=FALSE)
