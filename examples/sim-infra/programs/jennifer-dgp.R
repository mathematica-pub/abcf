...
genData <- function(
  ...
  driver = read_excel(path='/home/bcf-sim-infra/data/sim_study_config_medicare_and_edu.xlsx',
                      sheet=driver_sheet) %>%
  ...
  if(world=='medicare'){
    ...
    real_prac_size_q <- readRDS('/home/bcf-sim-infra/data/RealDataPracticeSizeQuantiles.RDS')
    ...
    real_edu_size_q <- readRDS('/home/bcf-sim-infra/data/sch-size-quantiles.RDS') %>%
      select(q_tested) %>%
      pull()
